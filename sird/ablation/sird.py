    ...
    def calc_loss(self, batch_data):
        ...
        if len(cat_p2_vars) > 0:
            ...
            for v in cat_p2_vars:
                name = v['name']
                partner = v['pair_partner']
                x_t = x_t_dict[name]  # 当前带噪样本
                x_proxy = p1_dict[name]  # Proxy
                x_true = batch_data[:, self.p2_slices[name]]
                pred_logits = model_out[name]
                pred_x0_probs = F.softmax(pred_logits, dim=-1)
                true_posterior = self._compute_posterior(x_t, x_true, x_proxy, t)
                model_posterior = self._compute_posterior(x_t, pred_x0_probs, x_proxy, t)
                log_model_posterior = torch.log(model_posterior.clamp(min=1e-10))
                kl = F.kl_div(log_model_posterior, true_posterior, reduction='batchmean')

                loss_cat_total += self.zeta * kl
            loss_total += loss_cat_total
        return loss_total

    def _compute_posterior(self, x_t, x_start, x_proxy, t):
        """
        计算 RDDM 的后验分布 q(x_{t-1} | x_t, x_start, x_proxy)
        公式: Posterior propto Likelihood * Prior

        Args:
            x_t: [B, K] 当前时刻的 One-hot 或 概率分布
            x_start: [B, K] x0 (True) 的 One-hot 或 预测概率
            x_proxy: [B, K] Proxy 的 One-hot
            t: [B] 时间步
        """
        # 1. 获取调度参数
        # 注意：你的代码中 alpha_bars 是 Proxy 的含量 (0 -> 1)
        # 我们用 lambda_t 表示 Proxy 含量
        lambda_t = self.alpha_bars[t].view(-1, 1)
        lambda_prev = self.alpha_bars[t - 1].view(-1, 1)

        # 你的代码中 beta_bars 是 incremental noise?
        # 我们需要计算单步转移的 beta。
        # 简单起见，可以用 lambda_t 和 lambda_prev 推算:
        # lambda_t = lambda_prev + beta * (1 - lambda_prev)
        # -> beta = (lambda_t - lambda_prev) / (1 - lambda_prev)
        beta_t = (lambda_t - lambda_prev) / (1 - lambda_prev + 1e-6)
        beta_t = beta_t.clamp(0, 1)  # [B, 1]

        # 处理 t=0 的边界情况
        lambda_prev[t == 0] = 0
        beta_t[t == 0] = 0

        # 2. 计算 Prior (从 x0 到 t-1 的分布)
        # q(x_{t-1} | x_0) = (1 - lambda_{prev}) * x_0 + lambda_{prev} * x_proxy
        prior = (1 - lambda_prev) * x_start + lambda_prev * x_proxy

        # 3. 计算 Likelihood (从 t-1 到 t 的逆向视角)
        # q(x_t | x_{t-1}) 的转置矩阵作用在 x_t 上
        # L = (1 - beta) * x_t + beta * (x_t^T x_proxy) * Ones
        # 解释: 如果 x_t 和 proxy 一致，那么所有来源都有可能；如果不一致，主要来源是 x_t 自己。

        # 计算点积 (x_t \cdot x_proxy)，结果为标量 [B, 1]
        # 假设 x_t 和 x_proxy 都是概率分布或 One-hot
        dot_prod = (x_t * x_proxy).sum(dim=1, keepdim=True)

        # 构造似然向量 [B, K]
        likelihood = (1 - beta_t) * x_t + beta_t * dot_prod
