...
tune_mice <- function(data, data_info, search_space,
                      log_path = "mice_tuning_log_OE.csv",
...
tm <- system.time({
  tune_srs <- tune_mice(
    ...
    log_path = "mice_tuning_results.csv",
    n_evals = 30, m = 5, folds = 3
  )
})
save(tm, file = "mice_tuning_time.RData")


mice_tuning_results <- read.csv("mice_tuning_results.csv")
mice_tuning_results <- mice_tuning_results %>%
  rowwise() %>%
  mutate(is_pareto = !any(
    mice_tuning_results$acc_loss <= acc_loss & 
      mice_tuning_results$dist_loss <= dist_loss & 
      (mice_tuning_results$acc_loss < acc_loss | mice_tuning_results$dist_loss < dist_loss)
  )) %>%
  ungroup()

pareto_front <- mice_tuning_results %>% filter(is_pareto == TRUE)
...
saveRDS(params, file = "../../data/Config/best_config_mice.rds")
