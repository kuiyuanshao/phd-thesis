...
do_mice <- function(dat, nm, digit, best_config) {
  ...
  repeat {
    ...
    if (!inherits(mice_imp, "error")) {
      ...
      save_file <- file.path(sim_root, type, nm, "mice", paste0(digit, ".RData"))
...
process_design <- function(design_name, digit, data) {
  out_path <- file.path(sim_root, type, design_name, "mice", paste0(digit, ".RData"))
...
if (length(timing_records) > 0) {
  ...
  csv_file <- file.path(sim_root, sprintf("%s_mice_iteration_times_task_%d.csv", type, task_id))
