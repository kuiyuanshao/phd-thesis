...
CONFIG_PROFILES = generate_config_profiles("sird")

print(f"Loaded {len(CONFIG_PROFILES)} profiles for the sird model.")
...
def setup_env(type, data_folder):
    ...
    save_dir = sim_root / type / data_folder / "sird"
...
def main():
    ...
    for i in range(start_idx, end_idx):
        ...
        if not save_path.exists():
            ...
            sird_mod = SIRD(base_config, data_info)
            sird_mod.fit(str(file_path))
            sird_mod.impute(save_path=str(save_path))
        else:
    ...
    if timing_data:
        csv_file_path = sim_root / f"{args.type}_sird_{args.profile}_iteration_times_task_{task_id}.csv"
