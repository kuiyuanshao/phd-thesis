...
cal_fun <- if (type == "SampleOE") calibrateFunOE else calibrateFunE

process_design <- function(design_name, digit, complete_data) {
  out_path <- file.path(sim_root, type, design_name, "raking", paste0(digit, ".RData"))
  ...
  if (!file.exists(out_path)) {
    ...
    tm <- system.time({
      if (design_name == "SRS") {
        rakingest <- cal_fun(samp, design = "SRS")
      } else {
        rakingest <- cal_fun(samp)
      }
    })
...
if (length(timing_records) > 0) {
  ...
  csv_file <- file.path(sim_root, sprintf("%s_raking_iteration_times_task_%d.csv", type, task_id))
