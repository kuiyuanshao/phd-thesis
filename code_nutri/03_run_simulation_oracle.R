...
sub_dirs <- c(
  "Neyman_ODS_UNVAL/oracle",
  "Neyman_INF_UNVAL/oracle"
)
...
do_oracle <- function(dat, nm, digit) {
  ...
  meth[] <- ""
  meth[c("ln_na_true", "ln_k_true", "ln_kcal_true", "ln_protein_true")] <- "norm"
  ...
  if (!inherits(oracle_imp, "error")) {
    cat(sprintf("System time: user=%.3fs sys=%.3fs elapsed=%.3fs\n",
                tm[["user.self"]], tm[["sys.self"]], tm[["elapsed"]]))

    save_file <- file.path(sim_root, nm, "oracle", paste0(digit, ".RData"))
    save(oracle_imp, file = save_file, compress = "xz", compression_level = 6)
    return(tm[["elapsed"]])
  }
...
process_design <- function(design_name, digit, complete_data) {
  out_path <- file.path(sim_root, design_name, "oracle", paste0(digit, ".RData"))
