...
class ResNetBlock(nn.Module):
    def __init__(self, d_main, d_hidden, dropout, d_cond, d_emb):
        ...
        self.cond_proj = nn.Linear(d_cond, d_main) #d_hidden // 2)

    def forward(self, x, cond, emb):
        identity = x
        x_in = self.normalization(x)
        x_in = x_in + self.emb_proj(emb) + self.cond_proj(cond)
...
class MlpBackbone(nn.Module):
    ...
    def forward(self, x_curr, cond, t):
        ...
        for block in self.blocks:
            cond_drop = self.dropout_cond(cond_in)
            x = block(x, cond_drop, emb)
        x = self.final_norm(x)
