...
kl_categorical <- c("batch_size", "layers", "channels", "diffusion_embedding_dim")
kl_continuous <- c("lr", "dropout", "loss_num", "loss_cat", "num_steps", "sum_scale", "weight_decay")
results_kl <- analyze_tuning(
  file_prefix = "./tuning_results/multinomial", 
  cat_params = kl_categorical, 
  cont_params = kl_continuous
)


cfg_continuous <- c("cfg_scale_num", "cfg_scale_cat", "cond_drop_prob")
results_cfg <- analyze_tuning(
  file_prefix = "./tuning_results/multinomial_cfg", 
  cat_params = NULL, 
...
ggsave("./simulations/Imputation_NominalCoverage_Barchart.png", width = 18, height = 18, limitsize = FALSE)
...
ggsave("./simulations/Imputation_OracleCoverage_Barchart.png", width = 18, height = 18, limitsize = FALSE)
...
ggsave("./simulations/Imputation_Coeff_Barchart.png", width = 18, height = 18, limitsize = FALSE)
...
ggsave("./simulations/Imputation_Bias_Boxplot.png", width = 18, height = 18, limitsize = FALSE)


ggplot(combined_resultCoeff_long) + 
  geom_violin(aes(x = Method, y = Percent_Bias, fill = Method), alpha = 0.6) + 
  geom_hline(yintercept = 0, lty = 2, color = "red", linewidth = 0.8) + 
  facet_wrap(~ Covariate, scales = "free", ncol = 4) + 
  scale_fill_viridis_d(option = "cividis") +
  labs(x = NULL, y = "Percentage Bias (%)") +
