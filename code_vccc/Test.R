...
samp <- read.csv("./data/Sample/SRS/0001.csv")
...
multi_impset <- list(multi_impset[[1]], multi_impset[[1]])
imp.mids <- imputationList(multi_impset)

ggplot() + 
  geom_density(aes(x = multi_impset[[1]]$VL_COUNT_BSL_LOG_v), colour = "red") + 
  geom_density(aes(x = data$VL_COUNT_BSL_LOG_v)) +
  geom_density(aes(x = samp$VL_COUNT_BSL_LOG_v))


ggplot() + 
  geom_point(aes(x = multi_impset[[1]]$VL_COUNT_BSL_LOG_v, y = data$CD4_COUNT_1Y_sqrt_v), colour = "red") + 
  geom_point(aes(x = data$VL_COUNT_BSL_LOG_v, y = data$CD4_COUNT_1Y_sqrt_v))
