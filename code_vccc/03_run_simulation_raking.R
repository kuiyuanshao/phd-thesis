lapply(c("survey", "svyVGAM", "survival", "dplyr", "stringr"), require, character.only = TRUE)
source("00_utils_functions.R")
source("./comparisons_tuning/raking/raking.R")
...
process_design <- function(design_name, digit, data) {
  ...
  if (!file.exists(out_path)) {
    ...
    tm <- system.time({
      rakingest <- list(
        gauss = calibrateFun(samp, design = design_name, outcome = "gauss"),
        bin   = calibrateFun(samp, design = design_name, outcome = "bin"),
        cox   = calibrateFun(samp, design = design_name, outcome = "cox")
      )
    })
