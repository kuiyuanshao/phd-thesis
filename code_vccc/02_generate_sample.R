lapply(c("stringr", "dplyr", "data.table"), require, character.only = T)
source("./00_utils_functions.R")

generateSamples <- function(data, n_phase2, seed, digit){
  set.seed(seed)
  nrow <- dim(data)[1]
  n_phase2 <- n_phase2
  covars <- names(data)[!(names(data) %in% c(data_info_vccc$phase1_vars, data_info_vccc$phase2_vars))]
  aux_vars <- rev(data_info_vccc$phase1_vars)
  target_vars <- rev(data_info_vccc$phase2_vars)
  model_formula <- "CD4_COUNT_1Y_sqrt_v ~ CD4_COUNT_BSL_sqrt_v + SEX"

  mainDir <- "./data"
  dir.create(file.path(mainDir, "Sample"), showWarnings = FALSE)
  dir.create(file.path(mainDir, paste0("Sample", "/SRS")), showWarnings = FALSE)
  dir.create(file.path(mainDir, paste0("Sample", "/RS")), showWarnings = FALSE)
  dir.create(file.path(mainDir, paste0("Sample", "/WRS")), showWarnings = FALSE)
  dir.create(file.path(mainDir, paste0("Sample", "/SFS")), showWarnings = FALSE)
  dir.create(file.path(mainDir, paste0("Sample", "/ODS_TAIL")), showWarnings = FALSE)
  dir.create(file.path(mainDir, paste0("Sample", "/Neyman_INF")), showWarnings = FALSE)
  dir.create(file.path(mainDir, paste0("Sample", "/Neyman_ODS")), showWarnings = FALSE)
  dir.create(file.path(mainDir, paste0("Sample", "/Neyman_ODS_UNVAL")), showWarnings = FALSE)
  dir.create(file.path(mainDir, paste0("Sample", "/Neyman_INF_UNVAL")), showWarnings = FALSE)

  file_srs <- paste0("./data/Sample/SRS/", digit, ".csv")
  if (!file.exists(file_srs)) {
    id_phase2 <- sample(nrow, n_phase2)
    data_srs <- data %>%
      dplyr::mutate(R = ifelse(1:nrow %in% id_phase2, 1, 0),
                    W = 1,
                    across(all_of(target_vars), ~ replace(., R == 0, NA)))
    write.csv(data_srs, file = file_srs)
  }

  file_ods_tail <- paste0("./data/Sample/ODS_TAIL/", digit, ".csv")
  if (!file.exists(file_ods_tail)) {
    order_outcome <- order(data[["CD4_COUNT_1Y_sqrt_v"]])
    id_phase2 <- c(order_outcome[1:(n_phase2 %/% 2)], order_outcome[(nrow - n_phase2 %/% 2 + 1):nrow])
    data_ods <- data %>%
      dplyr::mutate(R = ifelse(1:nrow %in% id_phase2, 1, 0),
                    W = 1,
                    across(all_of(target_vars), ~ replace(., R == 0, NA)))
    write.csv(data_ods, file = file_ods_tail)
  }

  file_rs <- paste0("./data/Sample/RS/", digit, ".csv")
  if (!file.exists(file_rs)) {
    modphase1 <- lm(as.formula(paste0(model_formula, " + ", paste0(aux_vars, collapse = " + "))), data = data)
    rs <- residuals(modphase1)
    order_residual <- order(rs)
    id_phase2 <- c(order_residual[1:(n_phase2 %/% 2)], order_residual[(nrow - n_phase2 %/% 2 + 1):nrow])
    data_rs <- data
    data_rs$rs <- rs
    data_rs <- data_rs %>%
      dplyr::mutate(R = ifelse(1:nrow %in% id_phase2, 1, 0),
                    W = 1,
                    across(all_of(target_vars), ~ replace(., R == 0, NA)))
    write.csv(data_rs, file = file_rs)
  }

  file_wrs <- paste0("./data/Sample/WRS/", digit, ".csv")
  if (!file.exists(file_wrs)) {
    pilot_prop <- 0.1
    n_pilot <- round(n_phase2 * pilot_prop)
    pilot_indices <- sample(1:nrow(data), n_pilot)
    pilot_data <- data[pilot_indices, ]

    remaining_indices <- setdiff(1:nrow(data), pilot_indices)

    var_formula <- as.formula(paste(target_vars[1], "~", "VL_COUNT_BSL_LOG_nv + CD4_COUNT_1Y_sqrt_v + CD4_COUNT_BSL_sqrt_v + SEX"))
    pilot_model <- lm(var_formula, data = pilot_data)
    outcome_model <- lm(model_formula, data = data[-pilot_indices, ])
    sd_pilot <- sd(resid(pilot_model))

    resid_outcome <- resid(outcome_model)
    wrs <- order(resid_outcome * sd_pilot)
    n_rest <- n_phase2 - n_pilot
    idx <- c(wrs[1:(n_rest %/% 2)], wrs[(nrow - n_pilot - n_rest %/% 2 + 1):(nrow - n_pilot)])
    id_phase2 <- c(pilot_indices, remaining_indices[idx])

    data_wrs <- data
    data_wrs$wrs <- 0
    data_wrs$wrs[pilot_indices] <- resid(pilot_model)
    data_wrs$wrs[-pilot_indices] <- resid_outcome * sd_pilot
    data_wrs <- data_wrs %>%
      dplyr::mutate(R = ifelse(1:nrow %in% id_phase2, 1, 0),
                    W = 1,
                    across(all_of(target_vars), ~ replace(., R == 0, NA)))
    write.csv(data_wrs, file = file_wrs)
  }

  file_sfs <- paste0("./data/Sample/SFS/", digit, ".csv")
  if (!file.exists(file_sfs)) {
    modphase1 <- lm(as.formula(model_formula), data = data)
    score <- residuals(modphase1) * data[[aux_vars[1]]]
    order_score <- order(score)
    id_phase2 <- c(order_score[1:(n_phase2 %/% 2)], order_score[(nrow - n_phase2 %/% 2 + 1):nrow])
    data_sfs <- data
    data_sfs$score <- score
    data_sfs <- data_sfs %>%
      dplyr::mutate(R = ifelse(1:nrow %in% id_phase2, 1, 0),
                    W = 1,
                    across(all_of(target_vars), ~ replace(., R == 0, NA)))
    write.csv(data_sfs, file = file_sfs)
  }

  file_ods <- paste0("./data/Sample/Neyman_ODS/", digit, ".csv")
  file_inf <- paste0("./data/Sample/Neyman_INF/", digit, ".csv")

  if (!file.exists(file_ods) || !file.exists(file_inf)) {
    quantile_split <- c(0.19, 0.81)
    outcome <- cut(data[["CD4_COUNT_1Y_sqrt_v"]], breaks = c(-Inf, quantile(data[["CD4_COUNT_1Y_sqrt_v"]], probs = quantile_split), Inf),
                   labels = paste(1:(length(quantile_split) + 1), sep=','))

    if (!file.exists(file_ods)) {
      data_ods_exactAlloc <- data
      data_ods_exactAlloc$outcome_strata <- as.numeric(outcome)
      alloc <- exactAllocation(data = data_ods_exactAlloc, stratum_variable = "outcome_strata",
                               target_variable = aux_vars[1], sample_size = n_phase2)
      weights <- table(data_ods_exactAlloc[["outcome_strata"]]) / alloc
      id_phase2 <- lapply(1:length(table(data_ods_exactAlloc[["outcome_strata"]])),
                          function(j){sample((1:nrow)[data_ods_exactAlloc[["outcome_strata"]] == j], alloc[j])})
      id_phase2 <- unlist(id_phase2)
      data_ods_exactAlloc <- data_ods_exactAlloc %>%
        dplyr::mutate(R = ifelse(1:nrow %in% id_phase2, 1, 0),
                      W = case_when(!!!lapply(names(weights), function(value){
                        expr(.data[["outcome_strata"]] == !!value ~ !!weights[[value]])
                      })),
                      across(all_of(target_vars), ~ replace(., R == 0, NA)))
      write.csv(data_ods_exactAlloc, file = file_ods)
    }

    if (!file.exists(file_inf)) {
      data_inf_exactAlloc <- data
      infl <- dfbeta(lm(as.formula(paste0(model_formula, " + VL_COUNT_BSL_LOG_nv + AGE_AT_MED_START_nv")),
                       data = data))[, 2]
      data_inf_exactAlloc$outcome_strata <- as.numeric(outcome)
      data_inf_exactAlloc$inf <- infl
      neyman_alloc <- exactAllocation(data_inf_exactAlloc, stratum_variable = "outcome_strata",
                                      target_variable = "inf",
                                      sample_size = n_phase2)
      neyman_weights <- table(data_inf_exactAlloc[["outcome_strata"]]) / neyman_alloc
      id_phase2 <- lapply(1:length(table(data_inf_exactAlloc[["outcome_strata"]])),
                          function(j){sample((1:nrow)[data_inf_exactAlloc[["outcome_strata"]] == j], neyman_alloc[j])})
      id_phase2 <- unlist(id_phase2)
      data_inf_exactAlloc <- data_inf_exactAlloc %>%
        dplyr::mutate(R = ifelse(1:nrow %in% id_phase2, 1, 0),
                      W = case_when(!!!lapply(names(neyman_weights), function(value){
                        expr(.data[["outcome_strata"]] == !!value ~ !!neyman_weights[[value]])
                      })),
                      across(all_of(target_vars), ~ replace(., R == 0, NA)))
      write.csv(data_inf_exactAlloc, file = file_inf)
    }
  }

  file_ods_unval <- paste0("./data/Sample/Neyman_ODS_UNVAL/", digit, ".csv")
  file_inf_unval <- paste0("./data/Sample/Neyman_INF_UNVAL/", digit, ".csv")

  if (!file.exists(file_ods_unval) || !file.exists(file_inf_unval)) {
    quantile_split <- c(0.19, 0.81)
    outcome_unval <- cut(data[[aux_vars[1]]], breaks = c(-Inf, quantile(data[[aux_vars[1]]], probs = quantile_split), Inf),
                         labels = paste(1:(length(quantile_split) + 1), sep=','))

    if (!file.exists(file_ods_unval)) {
      data_ods_unval <- data
      data_ods_unval$outcome_strata <- as.numeric(outcome_unval)
      alloc_unval <- exactAllocation(data = data_ods_unval, stratum_variable = "outcome_strata",
                                     target_variable = "CD4_COUNT_1Y_sqrt_v", sample_size = n_phase2)
      weights_unval <- table(data_ods_unval[["outcome_strata"]]) / alloc_unval
      id_phase2 <- lapply(1:length(table(data_ods_unval[["outcome_strata"]])),
                          function(j){sample((1:nrow)[data_ods_unval[["outcome_strata"]] == j], alloc_unval[j])})
      id_phase2 <- unlist(id_phase2)
      data_ods_unval <- data_ods_unval %>%
        dplyr::mutate(R = ifelse(1:nrow %in% id_phase2, 1, 0),
                      W = case_when(!!!lapply(names(weights_unval), function(value){
                        expr(.data[["outcome_strata"]] == !!value ~ !!weights_unval[[value]])
                      })),
                      across(all_of(target_vars), ~ replace(., R == 0, NA)))
      write.csv(data_ods_unval, file = file_ods_unval)
    }

    if (!file.exists(file_inf_unval)) {
      data_inf_unval <- data
      infl <- dfbeta(lm(as.formula(paste0(model_formula, " + VL_COUNT_BSL_LOG_nv + AGE_AT_MED_START_nv")),
                       data = data))[, 2]
      data_inf_unval$outcome_strata <- as.numeric(outcome_unval)
      data_inf_unval$inf <- infl
      neyman_alloc_unval <- exactAllocation(data_inf_unval, stratum_variable = "outcome_strata",
                                            target_variable = "inf",
                                            sample_size = n_phase2)
      neyman_weights_unval <- table(data_inf_unval[["outcome_strata"]]) / neyman_alloc_unval
      id_phase2 <- lapply(1:length(table(data_inf_unval[["outcome_strata"]])),
                          function(j){sample((1:nrow)[data_inf_unval[["outcome_strata"]] == j], neyman_alloc_unval[j])})
      id_phase2 <- unlist(id_phase2)
      data_inf_unval <- data_inf_unval %>%
        dplyr::mutate(R = ifelse(1:nrow %in% id_phase2, 1, 0),
                      W = case_when(!!!lapply(names(neyman_weights_unval), function(value){
                        expr(.data[["outcome_strata"]] == !!value ~ !!neyman_weights_unval[[value]])
                      })),
                      across(all_of(target_vars), ~ replace(., R == 0, NA)))
      write.csv(data_inf_unval, file = file_inf_unval)
    }
  }
}

if (file.exists("./data/params/data_sampling_seed.RData")){
  load("./data/params/data_sampling_seed.RData")
}else{
  seed <- sample(1:100000, 500)
  save(seed, file = "./data/params/data_sampling_seed.RData")
}

n <- 500
pb <- txtProgressBar(min = 0, max = n, initial = 0)
data <- read.csv("./data/data.csv")
for (i in 1:n){
  set.seed(i)
  setTxtProgressBar(pb, i)
  digit <- str_pad(i, nchar(4444), pad=0)
  generateSamples(as.data.frame(data), 304, seed[i], digit)
}
close(pb)