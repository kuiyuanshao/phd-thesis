...
do_mixgb <- function(samp, prms, nm, digit, data) {
  tm <- system.time({
    mixgb_imp <- mixgb(samp, m = 20, xgb.params = prms$params,
                       initial.fac = "sample", nrounds = prms$nrounds)
  })
  ...
  save(mixgb_imp, tm, file = save_file, compress = "xz", compression_level = 6)
  return(tm[["elapsed"]])
}
