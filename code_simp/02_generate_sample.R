...
generateSamples <- function(data, proportion, seed, digit){
  ...
  outcome <- cut(data[["Y"]], breaks = c(-Inf, quantile(data[["Y"]], probs = quantile_split), Inf), 
  ...
  infl = dfbeta(lm(as.formula(paste0(model_formula, " + ", aux_vars[1])), 
                   data = data))[, 2]
