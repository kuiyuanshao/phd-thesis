...
class BivariateTuner:
    def __init__(self, data, base_config, param_grid, data_info, n_splits=1):
        ...
        num_vars = self.data_info.get('num_vars', [])
        for c in num_vars:
            if c in self.data.columns:
                try:
                    self.data[c] = pd.to_numeric(self.data[c], errors='raise')
                except (ValueError, TypeError):
                    self.data[c] = pd.factorize(self.data[c])[0]
        if self.n_splits > 1:
            self.kf = KFold(n_splits=self.n_splits, shuffle=True, random_state=42)
            self.splits = list(self.kf.split(self.data))
            self.fold_histories = {i: {'acc': [], 'dist': []} for i in range(self.n_splits - 1)}
        else:
            self.kf = None
    ...
    def enforce_wgan_rules(self, config, p):
        if "pack" in p:
            batch_size = p.get("batch_size", config["train"]["batch_size"])
            batch_size -= batch_size % p["pack"]
            if "batch_size" in p:
        ...
        if "scale_hidden_dim" in p:
            base_hidden_dim = p.get("hidden_dim", config["model"]["generator"]["hidden_dim"])
            config["model"]["generator"]["hidden_dim"] = base_hidden_dim * p["scale_hidden_dim"]
        if "scale_layer" in p:
            base_layers = p.get("layers", config["model"]["generator"]["layers"])
            config["model"]["generator"]["layers"] = base_layers * p["scale_layer"]
        if "lr" in p or "scale_lr" in p:
            base_lr = p.get("lr", config["train"].get("Adam", {}).get("lr_d", 0.0002))
            scale_lr = p.get("scale_lr", 1.0)
            lr_g = base_lr / scale_lr
            for optimizer in ["Adam", "SGD"]:
                if optimizer in config["train"]:
                    config["train"][optimizer]["lr_d"] = base_lr
    ...
    def _get_trial_config(self, trial):
        ...
        for param_name, params in self.param_grid.items():
            ...
            if param_type == "cat":
                choices = params[1]
                flat_params[param_name] = trial.suggest_categorical(param_name, choices)
            elif param_type == "int":
                low, high = params[1], params[2]
                flat_params[param_name] = trial.suggest_int(param_name, low, high)
            elif param_type == "float":
                low, high = params[1], params[2]
                flat_params[param_name] = trial.suggest_float(param_name, low, high)
            elif param_type == "log_float":
                low, high = params[1], params[2]
                flat_params[param_name] = trial.suggest_float(param_name, low, high, log=True)
            elif param_type == "int_exp2":
                low_exp, high_exp = params[1], params[2]
                flat_params[param_name] = int(2 ** trial.suggest_int(param_name, low_exp, high_exp))
            elif param_type == "int_step":
                low, high, step_val = params[1], params[2], params[3]
                flat_params[param_name] = trial.suggest_int(param_name, low, high, step=step_val)
            else:
    ...
    def objective(self, trial):
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(self.splits):
            ...
            current_fold_acc = float(np.mean(m_mse) + np.mean(m_ce))
            current_fold_dist = float(np.mean(m_ed))

            if self.n_splits > 1 and fold_idx < (self.n_splits - 1):
                self.fold_histories[fold_idx]['acc'].append(current_fold_acc)
                self.fold_histories[fold_idx]['dist'].append(current_fold_dist)

                if len(self.fold_histories[fold_idx]['acc']) >= max(10, int(self.n_trials * 0.25)):
                    threshold_acc = np.nanpercentile(self.fold_histories[fold_idx]['acc'], 75)
                    threshold_dist = np.nanpercentile(self.fold_histories[fold_idx]['dist'], 75)

                    if current_fold_acc > threshold_acc and current_fold_dist > threshold_dist:
                        logger = optuna.logging.get_logger("optuna")
                        logger.info(
                            f"Trial {trial.number} pruned at fold {fold_idx}. "
                            f"Both Acc ({current_fold_acc:.4f} > {threshold_acc:.4f}) "
                            f"AND Dist ({current_fold_dist:.4f} > {threshold_dist:.4f}) are in the worst 25%."
                        )
