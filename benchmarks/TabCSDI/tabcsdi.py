...
class Tokenizer(nn.Module):
    ...
    def recover(self, Batch, d_numerical):
        ...
        if Batch.shape[1] > d_numerical:
            ...
            new_Batch_cat = torch.zeros([Batch_cat.shape[0], Batch_cat.shape[1]], device=Batch_numerical.device)
            ...
            for i in range(Batch_cat.shape[1]):
                ...
                diff = Batch_cat[:, i, :].unsqueeze(1) - emb_vec.unsqueeze(0)
                distances = torch.norm(diff, dim=2)
                nearest = torch.argmin(distances, dim=1)
                new_Batch_cat[:, i] = nearest + 1
...
class TabCSDI(nn.Module):
    ...
    def _build_networks(self, cont_list, num_cate_list):
        ...
        self.token_dim = self.config["diffusion"].get("token_emb_dim", 1)
    ...
    def fit(self, file_path=None, provided_data=None):
        ...
        for c in self.data_info.get('num_vars', []):
            if c in self.df_raw.columns:
                try:
                    self.df_raw[c] = pd.to_numeric(self.df_raw[c], errors='raise')
                except (ValueError, TypeError):
                    self.df_raw[c] = pd.factorize(self.df_raw[c])[0]
    ...
    def impute(self, m=None, save_path=None):
        ...
        target_batch_size = self.config['train']['eval_batch_size']
        dataloader = DataLoader(dataset, batch_size=target_batch_size, shuffle=False)
        self.eval()
        all_samples = []
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="Imputing Batches"):
                ...
                b_current = observed_data.shape[0]

                if b_current < target_batch_size:
                    pad_size = target_batch_size - b_current
                    observed_data = torch.cat([observed_data, observed_data[:pad_size]], dim=0)
                    observed_mask = torch.cat([observed_mask, observed_mask[:pad_size]], dim=0)
                    observed_tp = torch.cat([observed_tp, observed_tp[:pad_size]], dim=0)
                    gt_mask = torch.cat([gt_mask, gt_mask[:pad_size]], dim=0)

                side_info = self.get_side_info(observed_tp, gt_mask)
                batch_samples = self._reverse_diffusion(observed_data, gt_mask, side_info, m)

                if b_current < target_batch_size:
                    batch_samples = batch_samples[:b_current]
        ...
        for i in range(m):
            ...
            if self.config['model']['task'] == "ft":
                ...
                recovered = samples_i.squeeze(2).cpu().numpy()
    ...
    def _reverse_diffusion(self, observed_data, cond_mask, side_info, n_samples):
        ...
        for i in range(n_samples):
            current_sample = 0.8 #torch.randn_like(observed_data)
